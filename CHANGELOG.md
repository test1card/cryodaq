...
состоянию v0.64.0 и сверена с кодом; из git-индекса выведены процессные
артефакты; исправлена версия пакета в metadata.

### Changed

- README (en/ru), `PROJECT_STATUS.md`, `ROADMAP.md`, `docs/architecture.md`,
  `docs/operator_manual.md`, `docs/deployment.md`, `docs/instruments.md`,
  `docs/safety-operator.md`, `docs/alarms_tuning_guide.md`,
  `docs/operator/analytics-tab.md`, `THIRD_PARTY_NOTICES.md` — актуализация
  к v0.64.0: fail-closed safety, cold rotation + hot∪cold чтение,
  TSP watchdog modes, SQLite self-heal (Linux), аутентифицированные
  write-эндпоинты web API, зелёный CI. Руководство оператора и deployment
  описывают текущий shell `MainWindowV2` (ToolRail/overlays) вместо снятой
  v1-модели вкладок.
- `RELEASE_CHECKLIST.md` — добавлен обязательный шаг bump версии в
  `pyproject.toml`; состав GUI-поверхностей сверен с `gui/shell/tool_rail.py`.
- Комментарии/docstrings в `src/`, `tests/`, `tools/`, `plugins/` очищены от
  внутренних review-меток; техническое содержание сохранено (token-scoped
  diff-проверка + `compileall`).
- `CHANGELOG.md` — исторические записи очищены от внутренних процессных
  ссылок; даты, хэши и технические факты не тронуты.

### Fixed

- `pyproject.toml` version 0.60.0 → 0.64.1: релизы v0.61–v0.64 уходили со
  stale metadata — `importlib.metadata` и web-панель сообщали 0.60.0.

### Infrastructure

- Из git-индекса выведены (файлы остаются на диске): session-логи
  `docs/decisions/`, audit/handoff-отчёты design-system, процессные
  плейбуки/шаблоны, dev-контракты, локальное состояние инструментов.
- Doc-lint модульного индекса — module-level skip в свежем clone (CI).

### Test baseline

- 3 657 selected / 3 658 collected (1 deselected); изменения — документация
  и комментарии, исполняемый код не тронут.
...
- Модульный dev-индекс перестроен из docstring’ов (недоставало 80 живых
...
- 8d89f95, d4954ca — перестроенный модульный dev-индекс + doc-lint
...
## [0.55.5 — 0.55.16] — 2026-05-07/08 — autonomous run + external-audit batch (теги `archive/v0.55.*`)

### Audit batch v0.55.12 — v0.55.16

External audit on shipped tags v0.54.0 / v0.55.4 / v0.55.6 / v0.55.7
...
- **Скрытый баг с неактивными кнопками** — существовавший сигнал
  ...
  Поймал внешний аудит в цикле 1 FAIL/HIGH.

### Тесты

- 6 новых widget-тестов (15 → 21); 188 пройдено в более широкой
  регрессии.
- Внешний аудит: PASS, цикл 2 из 3.
...
- Внешний аудит: PASS, цикл 2 из 3 (цикл 1 — проблема с фикстурой на чистом
  checkout-е).
...
- Диагноз: зафиксирован в этой release-записи.
...
- Сводка глубокого аудита зафиксирована в этой release-записи.
- Консенсус 4 из 4 консультантов по Tier-1 находкам
  (независимое перекрёстное ревью).
- Ратификации архитектора: 8 вопросов, сессия 2026-05-04.
- Multi-verifier ship-аудит: первая линия PASS итерация 2; вторая линия
...
## [0.52.5] — 2026-05-04 — реальный баг live-панели аналитики (внешнее ревью)
...
- **`PressureCurrentWidget` никогда не получал live-данные**
  ...
  независимым перекрёстным ревью; подтверждена
...
- **`TemperatureOverviewWidget` показывал пустой график при
  ...
  катится за текущим временем. Корневая причина выявлена внешним
  ревью; подтверждена исходником pyqtgraph
...
Два предыдущих цикла анализа кода пришли к неверным выводам
(«expected behavior», «correctly wired»). Эскалировано до
независимого перекрёстного ревью согласно §v1.5.8.4. Ревьюеры:
внешний аудит (точная локализация бага), архитектурный проход
(анализ архитектурного дрейфа), нормализационный проход (вердикт о классе
нормализации), четвёртая линия (LOST — null API response). Синтез +
...
- **`TemperatureTrajectoryWidget._fetch_history()`** теперь
  ...
  в v0.47.4. Бонус-находка внешнего аудита F-X v3, закрыта.

### Расследование

Live-панели аналитики (`TemperatureOverviewWidget`,
`PressureCurrentWidget`) были объявлены «actually broken» в
рабочей спецификации. Анализ кода показал, что они **корректно подключены** —
...
- Фикс `TemperatureTrajectoryWidget`: бонус-находка внешнего аудита,
  F-X v3 2026-05-02.
...
Multi-verifier аудит (два независимых ревьюера) — 1 итерация.
P2-фиксы внешнего аудита применены (устаревший overlay чистится на путях
no-data/error).
...
- `CooldownAlarm` (`core/cooldown_alarm.py`): тревога по отклонению
  ...
  Закрывает пробел post-cooldown мониторинга, найденный в P1 внешнем
  ревью.
...
- Спецификация: зафиксирована в release-записи.
...
### Ссылки
- HF-спецификация зафиксирована в release-записи.
...
### Добавлено
- Pipeline-класс `AssistantQueryAgent`: classify → fetch →
...
### Аудит Phase F
- Внешний аудит: PASS после 3 fix-up циклов.
...
- `4acfd48` fix(f30): фиксы внешнего аудита, цикл 1.
- `8852786` fix(f30): фиксы внешнего аудита, цикл 2.
- `4a9800c` fix(f30): фиксы внешнего аудита, цикл 3.

## [0.46.1] — 2026-05-01 — F29 fix-up (перекрёстный аудит CF-2/CF-3/CF-5)

Patch-релиз с тремя фиксами, найденными в 8-линейном перекрёстном аудите
...
### Ключевые коммиты этого релиза
- `70bb588` fix(f29): cross-review audit findings CF-2 CF-3 CF-5.
...
- Smoke-харнесс F29 зафиксирован в release-записи.
...
### Ссылки
- Архитектура: `docs/architecture.md`, раздел local-assistant runtime.
- Спецификация: зафиксирована в release-записи.
- Smoke: зафиксирован в release-записи.
- Аудит: выводы CF-2/CF-3/CF-5 зафиксированы в v0.46.1.
...
- F28 Гемма — локальный LLM-агент полностью отгружен: Slice A
  ...
  подсказки) + Slice C (intro к отчёту эксперимента).
...
- Русскоязычный операторский диспатч (Telegram, operator log,
  GUI insight panel).
- Автогенерация DOCX intro отчёта эксперимента (формальный русский,
  200–400 слов).
...
- `config/agent.yaml` — новый namespace `agent.*` с `brand_name`
  и `brand_emoji`.
- `docs/architecture.md` — tracked архитектурная опора для фаз
  assistant v2 (F29-F33).
...
- Cycle 5: Slice C — отчёт эксперимента.
...
- 6 независимых audit-сессий в калибровочном журнале.
- Эмпирика: первая линия надёжна; вторая сильна на review-задаче
  (`max_tokens=8192`); третья over-flags; четвёртая
  деградирует; пятая падает.
...
### Главное
- 7 фич отгружены за один ночной спринт (F19-F25), закрыты
  все отложенные находки Task A и backlog полировки F3.
- Doc/process Phase A приземлены прямо в master ранее в сессии:
  governance v1.3 + multi-review consultation v1.1 +
  plugin disposition.
- Обе feature-ветки прошли внешний аудит:
...
- **F21 — гистерезисный deadband для тревог**:
  ...
  изначально-триггернувшие каналы (фикс внешнего аудита: сторонний канал
...
- **F19 — обогащение `ExperimentSummaryWidget`**: кликабельные
  ...
  P2-фикс внешнего аудита с 5000, которое покрывало лишь ~42 мин).
...
- Governance v1.3: §14.6 верификация галлюцинаций + §15
  реалии independent-review dispatch (6 подразделов: маршрутизация,
  задержки, бюджет, анти-паттерны).
- Multi-review consultation v1.1: §2.1 откалиброванная
...
- HF3: docstring `update_target()` — уточнение по slew-rate
  сходимости (внешний аудит T2 re-run CONDITIONAL → PASS).
- Disposition плагинов: автозагрузка dev-tooling выключена
...
- `4115703` — governance v1.3.
- `20b464b` — plugin disposition (dev-tooling disabled).
...
- **HF1**: уточнение docstring-а `update_target()` —
  подтверждённый delayed-update дизайн. CRITICAL-находка внешнего ревью
...
### Источник
Ночная сессия перекрёстного ревью 2026-04-29 — аудит архитектурных слепых
зон Task A (6 независимых линий × 4 задачи). Внешнее ревью отметило эти
находки. Обе верифицированы архитектором по реальному исходнику
до применения фикса.

Лог верификации зафиксирован в release-записи.
...
Реализация: 3-цикл ночной батч с внешним аудитом на цикл.
Quota независимого reviewer-а исчерпан ночью (MODEL_CAPACITY_EXHAUSTED на всех 4
...
- **B1 ZMQ idle-death** — H5 ПОДТВЕРЖДЕНО + ИСПРАВЛЕНО. См.
  разделы [0.39.0] и [0.36.0] в этом CHANGELOG.
...
Production hardening из ночного внешнего audit-батча (audit-03/04/05).
...
> Авторство 2026-04-21; мерджнуто в master
...
## [0.35.0] — 2026-04-24 — Governance параллельной разработки

### Главное

Governance-инфраструктура после хаоса параллельной разработки 2026-04-21
(дублирующие ветки, root-markdown поток, drift без
ведущего). Закрепляет maintainer-centric модель с явной
STOP-дисциплиной, autonomy-band-ом и правилами раскладки
артефактов.

### Добавлено

- `5286fa2` — governance v1.1: maintainer-centric
  role-матрица, branch-дисциплина, artifact-layout, STOP-дисциплина
  (§13), autonomy band (§13.5).
- `9a1a100` — consultation + negative-space рабочие правила.
- `587bea8` — `.gitignore`: исключение legacy workspace-ов параллельной
  разработки и `.worktrees/`.
...
### Сегодня — сессия 2026-04-20 (handoff → внешний reviewer)

Это плотная рабочая запись, не формальный релиз. Полный
handoff-контекст зафиксирован в release-записи; следующий
...
- `be51a24` — `zmq: ephemeral REQ per command + cmd-channel
  ...
  externally proposed B1 fix plan: per-command ephemeral REQ socket in
  ...
  **Does NOT fix B1 — Stage 3 diag tools still reproduce it.**
  ...
  Guide ch.4 canonical reliable req-reply pattern. External review's
...
- **B1 still unresolved.** GUI command channel silently dies
  ...
  transport experiment is the next attempt. Workaround in place:
...
- Independent-review development stack adopted (2026-04-20 afternoon).
  Primary quota exhausted; temporary proxy routing used for the
  coming ~4-5 days. Separate review quotas remain isolated.
  Operational details and identity-leakage warnings зафиксированы
  в release-записи.
...
- **Phase 2e Stage 1.** Стриминговый архив Parquet, пишется при
  ...
  внешним аудитом H.7 (стриминговая запись,
...
- **Интеграция Graphify knowledge graph.** Постоянная структурная
  ...
  рабочие сессии через хук `UserPromptSubmit` (62 мс на запуск).
...
- **Tier 1 Fix A — канонизация канала калибровки (`a5cd8b7`).**
  ...
  failure-ответ вместо падения. Закрывает внешний аудит round 2 NEW
...
- **Tier 1 Fix B — изоляция исключений subscriber-ов DataBroker
  ...
  от друга. Закрывает внешний аудит round 1 finding B.1 / round 2
...
- **Phase 2d hardening безопасности и персистенса (14 коммитов).**
  ...
  (внешнее ревью R2). Мониторинг heartbeat-а RUN_PERMITTED. Fail-closed
...
- **Сломанный pytest-вызов в `inject_context.py` (`f6fe4b9`).**
  ...
  `"Tests: no output"` на каждом prompt-е рабочей сессии. Заменён на
  62-мс версию, использующую git-метаданные + god-узлы graphify.
- **Внешний аудит R1 finding A.1 — регрессия атомарности throttle-а
...
- **Project-level hook.** Project settings содержат
  `PreToolUse` для `Glob|Grep`, напоминающий сначала читать
  `graphify-out/GRAPH_REPORT.md`.
- **Трёхслойный review-pipeline**, заведённый в Phase 2d:
  tactical + external second-opinion + architectural. 14
  коммитов, 17 внешних ревью, 2 архитектурных раунда.
...
- **Чувствительность к wall-clock** в `alarm_providers.py` и
  `channel_state.py` (`time.time()` vs `monotonic()`). Внешний аудит R2
  подтвердил находку, ещё не закрыта.
- **Блокирующая генерация отчётов** — синхронный
  `subprocess.run()` для LibreOffice. Внешний аудит R1 E.1, всё ещё
  открыт.
...
- `dafdd99` docs: post-merge PROJECT_STATUS и dev-индекс updates.
- `f6fe4b9` infra: graphify setup + inject_context hook efficiency fix.

Подробная коммит-история Phase 2d (14 коммитов): см. секцию
«Phase 2d commits» в `PROJECT_STATUS.md`. Audit-трейл внешнего ревью
зафиксирован в этом CHANGELOG.
...
внешнего ревью Round 2, завершение fail-closed конфигурации, официальное
...
- **Ruff lint** — накопленный долг сокращён с 830 до 445 ошибок
  (`efe6b49`, 132 файла).
- **Внешнее ревью R2: `_fault()` ordering** — post-mortem log callback вызывается
  ДО optional broker publish, устраняя escape path при отмене.
- **Внешнее ревью R2: calibration state mutation** — `prepare_srdg_readings()`
...
модульного dev-индекса.

### Добавлено

- **DOC_REALITY_MAP** — сверка 28 организационных документов
  против 62 non-GUI модулей (парное ревью).
- **Модульный dev-индекс** — расширение покрытия с ~34% до ~70%.
- **Safety FSM в dev-индексе** — исправлены инварианты и добавлено
...
- **Deep audit A** — 1 240 строк post-2c анализа.
- **Deep audit B** — 763 строки overnight-анализа.
...
- **Внешние audit findings** — `plugins.yaml` латинская T,
...
## [0.10.0] — 17-03-26 — RC-слияние (внешний аудит)

Одиночный merge commit, интегрирующий работу из параллельной ветки разработки.
...
- **RC merge** — `dc2ea6a`, масштабное слияние всех backend и
...
- **Архитектурный контракт** в модульном dev-индексе — описание двухпроцессной
