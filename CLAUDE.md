...
**SQLite requirement:** the runtime must use SQLite **≥ 3.51.3** (or a
backport-safe 3.44.6 / 3.50.7). `SQLiteWriter._check_sqlite_version` hard-fails
on the March-2026 WAL-reset corruption range `[3.7.0, 3.51.3)`. This now
self-heals on Linux: `storage/_sqlite.py` picks the sqlite3 implementation once
at import — if the stdlib SQLite is in-range it transparently falls back to the
bundled `pysqlite3-binary` (a base dependency, Linux-only marker; macOS ships no
wheels and its stdlib is expected safe). Every runtime importer takes sqlite3
from `cryodaq.storage._sqlite` so all connections to one DB share a single SQLite
library. Manual remediation is only needed if BOTH the stdlib and the fallback
are unsafe/absent — then use a Python linked against a safe SQLite (e.g.
Homebrew's `python@3.13`/`python@3.14`, SQLite ≥ 3.53) rather than setting
`CRYODAQ_ALLOW_BROKEN_SQLITE=1`. The bypass flag is a last-resort acknowledgement,
not the fix.
...
- `src/cryodaq/storage/_sqlite.py` — single source of the sqlite3 implementation: auto-fallback to bundled pysqlite3 (Linux) when the linked SQLite is in the WAL-reset corruption range; owns the version predicate + range/backport constants used by the gate
