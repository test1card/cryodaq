...
**SQLite requirement:** the interpreter must link SQLite **≥ 3.51.3** (or a
backport-safe 3.44.6 / 3.50.7). `SQLiteWriter._check_sqlite_version` hard-fails
on the March-2026 WAL-reset corruption range `[3.7.0, 3.51.3)`. Some prebuilt
interpreters bundle an old SQLite (e.g. the python.org macOS framework build
shipped 3.50.4) — use a Python linked against a safe SQLite (e.g. Homebrew's
`python@3.13`/`python@3.14`, which link SQLite ≥ 3.53) rather than setting
