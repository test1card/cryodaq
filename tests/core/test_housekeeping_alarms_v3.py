...
def test_load_critical_simple_threshold(tmp_path):
    ...
    assert re.escape("Т20") not in patterns, "INFO-level alarm channel must NOT be protected"
...
def test_additional_condition_walked(tmp_path):
    ...
    assert re.escape("P3") in patterns, "additional_condition.channel must be walked recursively"
...
def test_throttle_protects_alarms_v3_channels(tmp_path):
    ...
    assert len(out_t11) == 1, "Т11 should be protected (alarms_v3 critical) and bypass throttling"
