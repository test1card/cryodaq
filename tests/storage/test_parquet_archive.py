...
def _make_readings(n: int = 100, channels: int = 3) -> list[dict[str, Any]]:
    ...
    base_ts = datetime(2026, 3, 21, 12, 0, 0, tzinfo=timezone.utc)
    readings = []
    for i in range(n):
        for ch_idx in range(channels):
            ts = datetime.fromtimestamp(
                base_ts.timestamp() + i * 0.5, tz=timezone.utc
            )
            readings.append({
                "timestamp": ts,
...
def test_timestamp_precision(tmp_path: Path) -> None:
    ...
    precise_ts = datetime(2026, 3, 21, 12, 0, 0, 123456, tzinfo=timezone.utc)
    readings = [{
