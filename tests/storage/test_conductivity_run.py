...
def test_terminal_accepted_count_must_equal_checkpoint_count(tmp_path) -> None:
    path = tmp_path / "checkpoint-count-mismatch.csv"
    writer = ConductivityRunWriter(
        path,
        run_id="checkpoint-count-mismatch",
        started_at=datetime(2026, 8, 27, 12, tzinfo=UTC),
        parameters=_bound_parameters(),
    )
    writer.append_binding("experiment-a")
    writer.append_point(_point())
    writer.append_point({**_point(), "timestamp_utc": "2026-08-27T12:00:02+00:00"})
    ...
    terminal_index = next(index for index, line in enumerate(lines) if "conductivity_run_terminal" in line)
    terminal = json.loads(lines[terminal_index][len(module._COMMENT_PREFIX) :])
    terminal["accepted_point_count"] = 1
    lines[terminal_index] = module._COMMENT_PREFIX + json.dumps(
        terminal,
        ensure_ascii=False,
        separators=(",", ":"),
    ...
    with pytest.raises(ConductivityRunFormatError, match="does not match its checkpoint count"):
        read_conductivity_run(path)
...
def test_checkpoint_real_fsync_then_raise_keeps_point_unaccepted(tmp_path, monkeypatch) -> None:
    ...
    assert "trailing_write_outcome" not in snapshot.terminal
...
def test_manager_rejects_run_record_count_different_from_terminal(tmp_path) -> None:
    manager_root = tmp_path / "manager"
    manager_root.mkdir()
    instruments = manager_root / "instruments.yaml"
    instruments.write_text("instruments: []\n", encoding="utf-8")
    ...
    path = tmp_path / "count-conflict.csv"
    writer = ConductivityRunWriter(
        path,
        run_id="count-conflict",
        started_at=datetime(2026, 8, 27, 12, tzinfo=UTC),
        parameters=_bound_parameters(),
    )
    writer.append_binding("experiment-a")
