...
async def test_emergency_off_logs_critical_on_verify_failure(caplog):
    ...
    relevant = [r for r in caplog.records if "still reports output" in r.message or "readback failed" in r.message]
...
async def test_emergency_off_handles_query_exception(caplog):
    ...
    relevant = [r for r in caplog.records if "verify failed" in r.message.lower()]
...
async def test_incomplete_transport_close_blocks_keithley_disconnect_and_reconnect() -> None:
    from cryodaq.drivers.instruments.keithley_2604b import TransportTeardownIncompleteError
