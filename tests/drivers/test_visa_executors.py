...
@pytest.mark.asyncio
async def test_usbtmc_cancelled_close_with_successful_handle_settlement_enables_recovery(monkeypatch) -> None:
    import asyncio
    ...
    async def _successful_open(_resource_str: str) -> None:
        transport._resource = _Resource()
        transport._rm = _Manager()

    monkeypatch.setattr(transport, "_settle_open", _successful_open)
    await transport.open("USB0::RECOVERED::INSTR")
    assert transport._query_desynchronized is False
    await transport.close()
