...
def test_timeout_layer_ordering_invariant() -> None:
    ...
    assert helper_s < envelope_s, f"helper {helper_s}s must fire before ZMQ slow envelope {envelope_s}s"
    assert envelope_s <= client_s, f"ZMQ slow envelope {envelope_s}s must fire before client reply {client_s}s"
...
def test_helper_default_absorbs_cold_start() -> None:
    ...
    sig = inspect.signature(_handle_assistant_query_command)
    helper_s = sig.parameters["timeout_s"].default
    assert helper_s >= 40.0, f"helper default {helper_s}s does not cover Ollama cold-start"
