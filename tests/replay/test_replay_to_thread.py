...
@pytest.mark.asyncio
async def test_sqlite_replay_loads_in_thread(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("cryodaq.replay_engine.sources._load_db_rows", _fake_load)
    ...
    assert seen_threads[0] != main_thread, "SQLite load ran on the asyncio loop thread"
