...
def test_start_refuses_to_replace_ownership_when_old_reply_consumer_survives(
    ...
    assert bridge._process is process
    # Cleanup failure must leave a fresh quarantined queue detached from the
    # old producer; retaining the old queue would allow stale cuts to return
    # if a later retry clears the shared shutdown event.
    assert bridge._snapshot_queue is not old_snapshot_queue
