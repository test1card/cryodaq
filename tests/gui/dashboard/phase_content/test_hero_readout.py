...
def test_hero_readout_annotation_optional(app):
    ...
    assert not w._annotation_label.isHidden(), "annotation_label should be visible when annotation is set"
    w.set_value(4.21, "K")
    assert w._annotation_label.text() == "", (
        f"Expected empty text after clearing annotation, got {w._annotation_label.text()!r}"
    )
    assert w._annotation_label.isHidden(), "annotation_label should be hidden when annotation is None"
