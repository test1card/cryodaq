...
> Живой документ. Обновлён 2026-07-17 для checkpoint-кандидата, влитого в
> `master` 2026-07-31, поверх релизной основы v0.64.1. Описанные defaults не
> являются уже выпущенным поведением и требуют гейтов из `PROJECT_STATUS.md`.
>
...
critical_channels:              # обязательны для перехода в RUN
  - "Т11 .*"                    # 1-я ступень GM-cooler (positionally fixed)
  - "Т12 .*"                    # 2-я ступень, холодная точка (positionally fixed)
...
|---|---|---|
| `critical_channels` | 2 regex (`Т11`, `Т12`) | Сейчас safety FSM гейтится только на positionally-fixed reference-каналы. Не добавляй мобильные per-experiment датчики без отдельного физического обоснования: это создаёт ложные fault'ы при перестановке датчиков. |
...
- `stop_source` — мягкая остановка источника тока через
  `SafetyManager.request_stop()`. Без нового fault latch, но с обязательным
  подтверждением OFF обоих каналов.
- `warning` — операторское предупреждение о качестве данных. Оно не получает
  полномочий SafetyManager и не отключает источник.
...
- `detector_warmup` работает в любой фазе и lifecycle-состоянии как
  наблюдательное предупреждение. `source_overtemp` получает полномочия на
  остановку только в активном lifecycle источника; в idle остаётся `ARMED`.
- **Regex pattern** — `"Т[1-8] .*"` матчит `"Т1 Криостат верх"`,
  `"Т2 ..."` и т.д. Точка с пробелом важна — это разделитель
  `channel_id <space> description`.
- **Cooldown** ограничивает повторные уведомления, а не защитное действие.
  Наблюдательное предупреждение остаётся активным до безопасного показания
  после окончания cooldown, поэтому шум около порога не создаёт повторный
  `alarm_fired`. Защитный interlock после acknowledge снова выполняет действие,
  даже если нарушение сохраняется внутри cooldown.
...
**Setpoint T12** — задаётся в `custom_fields` карточки эксперимента и
остаётся доступен провайдеру setpoints. В текущем shipped
`alarms_v3.yaml` (v0.64.0) активных правил, которые его потребляют,
нет: старые `detector_drift`, `detector_unstable`, `cooldown_stall`
удалены в пользу physical alarms.
...
## Текущий состав v2 алармов (16 штук)
...
| Имя | Условие | Уровень |
|---|---|---|
| `shield_warming` | Т12 rate >0.5 K/мин за окно 300с | WARNING |
...
```yaml
# Сейчас в shipped alarms_v3.yaml отсутствуют.
# Если возвращать: setpoint T12 доступен как T12_setpoint (default 4.2K).
```
...
**3.3 Cooldown stall**
```yaml
# В shipped alarms_v3.yaml отдельного cooldown_stall нет.
# Ближайший контур — CooldownAlarm в config/physical_alarms.yaml.
```
...
- 2026-07-08: синхронизация с v0.64.0: `critical_channels` только Т11/Т12,
  16 активных v2-алармов, physical alarms вынесены в отдельный слой.
