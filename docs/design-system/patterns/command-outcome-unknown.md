...
| `main_window_v2.py` (shell orchestrator; drives `phase_aware_widget.py`, does not implement the pattern) | Engine connection lost: the disconnect path calls `phase_widget.set_operation_state("unknown", unavailable_reason)` so an optimistic phase display cannot outlive the connection that justified it | the widget's own exit — an authoritative `experiment_status` read reconciles it | canonical (a), through the instance it drives | proactive | forces unknown on disconnect rather than leaving the last known phase on screen; `set_mutation_enabled(False)` is issued in the same block, so the unknown state cannot be acted on | every call is wrapped in `attempt(...)`, so one failing widget cannot abort the rest of the disconnect fan-out |
...
## Changelog
- 2026-08-10 (v4.0.5): Added `main_window_v2.py` to the instance table. The
  shell drives the phase widget into outcome-unknown when the Engine connection
  is lost. It orchestrates an existing instance and implements no new pattern.
