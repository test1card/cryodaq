...
**Session:** v4.0.4 operator-truth hardening
...
62. **Design system remains flat-token in the current v4.0.4 line** — the v2.0.0
...
63. **STONE_* remains deprecated/read-only in the current v4.0.4 line** — neither
...
**Design system v4.0.4 — informative and intentionally beautiful CryoDAQ composition is a breaking, corpus-wide GUI gate; panoramic dashboard home, descriptor-qualified identity, exact audible-annunciation ownership, and the supplemental atomic briefing are implemented, while remaining migration and external evidence stay open in `GUI_MIGRATION_INVENTORY.md`.** Existing
