...
- Moved audible alarm presentation to one bounded, engine-projection controller.
  TopWatchBar and BottomStatusBar no longer infer sound from alarm rows or a
  safety-state string. Malformed, older, or equivocal projection replies retain
  the accepted audible activation; an engine replacement starts fail-loud.
- Made knowledge-index maintenance visibly offline-only. The operator UI has no
  rebuild or rebuild-status command path; `cryodaq-rag-index` is the explicit
  maintenance route.
- Clarified that BottomStatusBar is a passive backend-evidence presenter: disk
  thresholds and provenance belong to DiskMonitor, and the strip performs no
  filesystem probing.
