...
| Data disk | backend `DiskMonitor` evidence through `set_disk_evidence` | retained `Диск N ГБ` with source/state detail |
...
- disk evidence is an independent backend stream, not an atomic snapshot field;
