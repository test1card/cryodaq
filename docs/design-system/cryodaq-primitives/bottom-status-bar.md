...
5. Any safety state containing `fault` uses neutral `FOREGROUND` body text
...
6. Disk free space below 2 GiB uses neutral `FOREGROUND` body text with a
   `STATUS_FAULT` left border; 2 GiB through less than 10 GiB uses
   `STATUS_CAUTION`; 10 GiB or more is muted technical information.
...
| contains `fault` | `FOREGROUND` text + `STATUS_FAULT` left border | fault evidence; exact `fault_latched` repeats the beep |
