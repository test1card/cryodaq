...
9. **Legacy token usage** (`TEXT_PRIMARY` / etc.) or legacy helpers (`PanelHeader` / `StatusBanner` / `apply_*_style` / `setup_standard_table` / `add_form_rows`). All forbidden. `TEXT_DISABLED` remains permitted for `QPushButton:disabled` styling.
