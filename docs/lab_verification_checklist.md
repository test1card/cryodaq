...
- Фатально: любое значение в `[3.7.0, 3.51.3)` — движок откажется писать.
  Пример известного плохого билда: python.org macOS framework везёт `3.50.4`.

**Если версия плохая.** Поставить Python, слинкованный с безопасным SQLite
(например Homebrew `python@3.13` / `python@3.14` — линкуют SQLite ≥ 3.53),
и запускать движок им. `CRYODAQ_ALLOW_BROKEN_SQLITE=1` — только
крайняя мера-подтверждение (осознанный обход), **не** исправление.
