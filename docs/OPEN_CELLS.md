...
| OC-020 | substrate | Candidate immutability during execution | **STILL OPEN — the bar was raised, not closed.** The export is now made read-only for the duration of execution, which defeats a naive mutate-run-restore. It does not defeat a deliberate one: the candidate command runs as the same user, so it can `chmod` the tree back to writable, mutate, execute the mutated bytes, restore both the original bytes and the original permissions, and receive a PASS receipt. Reproduced directly against the fix. | Reviewers can still receive a green exact-candidate receipt, carrying the correct commit, tree and manifest digests, for bytes that were not immutable throughout execution. | `tools/candidate_evidence.py` `_set_export_read_only`; `tests/governance/test_candidate_evidence_binding.py`. Reproduction: chmod-restore bypass yields `returncode=0`, `EXECUTED_MUTATED_BYTES` in stdout, receipt PASS. | POSIX mode bits cannot bind a process running as their own owner. Closure needs a boundary the candidate cannot lift: a deny-write ACL that also denies WRITE_DAC on Windows, a read-only bind mount, or execution under a distinct principal. | **No.** The mutate-execute-restore interval remains reachable. | An immutability boundary the candidate process cannot revoke. BLOCKS-DEPLOYMENT. | CI/evidence owner |
| OC-021 | substrate | Montana candidate gate | **CLOSED.** The gate now runs as an unconditional step in every OS/suite matrix job, with no `if` and no `continue-on-error`, and the job's final enforcement step requires `steps.montana-candidate-gate.outcome == success` under `if: always()` — so a skipped gate also fails the job. Deletion was rejected as closure because this plan relies on its contract. | — | `.github/workflows/main.yml` step `Enforce Montana candidate gate`; regression guard in `tests/test_ci_candidate_evidence.py` asserts the wiring and ordering; a behavioural test executes the CLI and observes exit 1 on a violating bundle and exit 0 on the control. | — | **Yes.** The validator can now refuse a real candidate. | Verified independently at the frozen head under P7. | CI/governance owner |
...
| OC-035 | substrate | Validators execute from candidate-controlled bytes | Both the Montana candidate gate and the new CI partition-execution proof run as `python -m tools.<validator>` from the candidate checkout, so **a candidate supplies the bytes of its own judge**. Nothing structural stops a commit from weakening the validator it will then be judged by. | Required merge evidence can be made to appear executed and passed when it was not: rename an echo step as the candidate step and emit forged canonical receipts into both the artifact and the job log, or simply relax the validator in the same commit. Overlapping same-OS jobs also carry no cryptographic job-ID field, so receipt-to-job correlation is time-window plus log matching — strong against accident, not against a malicious workflow author. | `.github/workflows/main.yml` step `Enforce Montana candidate gate`; `tools/ci_partition_execution_proof.py`; the verifier's own reported attack surface. | Closure needs the gate job to execute verifier bytes from a **protected, pinned workflow ref** rather than from the candidate checkout, plus a cryptographic job binding in the execution receipt. Part code (`main.yml` must call a pinned reusable workflow), part repository settings. | **No.** A weakened validator passes silently. | Pin the judge outside the candidate. BLOCKS-DEPLOYMENT. | CI/governance owner |
...
- Canonical execution-root selection replaced the three-way string binding, and
  the export is now held read-only while the candidate runs. OC-020 is narrowed
  to a deliberate mutation: the candidate can restore write permission itself,
  so the window is smaller but still reachable. **Raising a bar is not closing a
  blocker, and this row is not counted as closed.**
...
**Blockers: OC-020 (4), OC-033 (1), OC-035 (4).** OC-034 is deliberately NOT counted as a code blocker: its
