...
| OC-040 | C2 | DISCLOSED | Engine YAML loaders inherit PyYAML's mutable parsing tables | **The realistic ordering is now closed; one ordering is not.** Subclassing `yaml.SafeLoader` shares five mutable class attributes BY REFERENCE — `yaml_constructors`, `yaml_multi_constructors`, `yaml_implicit_resolvers`, `yaml_path_resolvers`, `bool_values` — so any library that calls `yaml.SafeLoader.add_constructor(...)` or `add_implicit_resolver(...)`, which is documented public PyYAML use and not an attack, decided what three engine loaders parse. `src/cryodaq/storage/channel_descriptors.py`, `src/cryodaq/periodic_config.py` and both legacy paths in `src/cryodaq/core/physical_alarms_config.py` (which used bare `yaml.safe_load`, i.e. `yaml.SafeLoader` itself) now hold their own copies. **What is NOT established is the pre-import ordering:** the copies are taken at class-definition time, so a library imported ahead of `cryodaq.core` that registers at its own import time still has its value copied in. | With production reverted and the library registering after import, a valid physical-alarms document returned `cold_channel`, `warm_channel` and `reference_temp_channel` as the library's substituted string, SUCCESSFULLY and with `escalate_to_safety` still false: the CooldownAlarm and VacuumGuard bind to channel identities that exist nowhere, with no diagnostic. Poisoning by ordering ACCIDENT is the realistic shape here, not by intent. | `tests/core/test_yaml_loader_table_ownership.py`. Every ordering assertion runs in a FRESH INTERPRETER, because by collection time `cryodaq` is already imported and an in-process test can only exercise the easy ordering. Red-before was verified against REVERTED PRODUCTION for each of the three modules in turn, each turning exactly its own guards red and each file restored byte-identical afterwards. `test_library_registration_before_import_is_a_disclosed_limit` asserts the OPEN half, so the disclosure cannot silently stop being true in either direction. | Closing the pre-import half needs constructors DEFINED IN THE PACKAGE rather than copied, as `cryodaq.lab_profile` does. That is a larger change to loaders whose vocabulary must stay wide, and it is deliberately not attempted in this change. | **No for the pre-import ordering** — a poisoned value parses successfully and silently. **Yes for the post-import ordering**, which now parses as authored. | NONBLOCKING — the remaining half is disclosure debt under the rubric at lines 30–36: the post-import half needed only ordinary library behaviour and is fixed; the pre-import half needs a package imported ahead of `cryodaq`, which is host-level control of import order. Revisit if a descriptor-defined vocabulary lands for these loaders. | Storage + engine-wiring owner |
