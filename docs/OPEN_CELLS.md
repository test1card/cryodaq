...
At the owner-authorized Cycle 2 planning snapshot, `master` had advanced beyond
`f5d6434d`; the exact distance was not bound to immutable evidence in this
document and is not asserted as current. The Cycle 1 judge SHA is no longer the
pin for Cycle 2; its repaired successor is pending and no SHA is claimed here.
The protected CI lock is version-pinned without artifact hashes.
It is an owner-authored, candidate-compatible snapshot pending independent
review and hosted proof; it is **not reviewed evidence**.

The guard-root repair currently in the working tree is authored, not reviewed:
Git object resolution is rooted in the repository under validation, and a
protected-path run without the required candidate repository refuses rather
than silently downgrading. The ordinary CI exact-checkout guard separately
resolves Git objects; only the sealed export subrun skips resolution when no
repository is available, so ordinary CI coverage was not compromised.
...
| OC-035 | substrate | Default-branch validator authority | Validators execute from the protected default-branch judge, and real hosted OIDC/REST job binding has been observed. Cycle 2 must re-establish those facts for the pending repaired default-branch commit and the exact candidate. | The ratified checkpoint protects against accidental or agent-induced validator weakening. It does not resist a malicious default-branch commit, compromised runner or GitHub identity, or a same-process Byzantine candidate. | `.github/workflows/protected-ci-evidence-gate.yml`; `tests/governance/test_protected_ci_evidence_gate.py`; the Cycle 2 review and hosted receipts are pending. Ordinary CI also runs its exact-checkout guard with Git resolution; the exported subrun's no-repository skip does not remove that ordinary-CI coverage. | The working-tree guard-root repair is authored but not independently reviewed or proven in hosted Cycle 2 execution. The repaired default-branch commit and pin do not yet exist. | **Fail-closed behavior is authored, not yet accepted:** a protected-path run without the candidate repository refuses instead of downgrading. | **Checkpoint prerequisite until P1, P7, and the candidate-bound check succeed; then satisfied only for the ratified checkpoint threat model.** A required-check setting enabled after the requested fast-forward protects subsequent changes; it did not gate that fast-forward. | CI/governance owner |
| OC-036 | substrate | Protected evidence-producer substitution | The protected producer and pytest plugin come from the protected default-branch judge, so candidate copies cannot accidentally weaken them. The candidate tests still execute in the same pytest process and OS account. | The checkpoint covers accidental or agent-induced producer substitution. Deliberate plugin mutation, protocol forgery, background tampering, or equivalent hostile same-authority behavior is outside its claim. | `tools/ci_candidate_runner.py`; `.github/workflows/protected-ci-evidence-gate.yml`. The authored working-tree repair roots producer-object verification in the candidate repository and pins the protected CI lock, but it is pending independent review and hosted proof. The protected lock is version-pinned without artifact hashes and is an owner-authored candidate-compatible snapshot, not reviewed evidence. | Same-process Byzantine resistance remains open; the hashless lock also does not establish artifact immutability. | **Yes** for the specified accidental/agent-induced producer-substitution attack after Cycle 2 prerequisite evidence; **no claim** for hostile same-process behavior. | **Checkpoint prerequisite for the specified accidental/agent-induced producer-substitution attack; residual Byzantine same-process resistance is BLOCKS-DEPLOYMENT disclosure debt.** | CI/evidence owner |
...
| OC-038 | substrate | Default-branch protected evidence gate qualification | The protected workflow is present on `master`; deployment absence was the Cycle 1 defect and is no longer the current statement. Cycle 1 nevertheless ended `NOT_PR_READY` after the guard-root defect was found. | OC-035 and OC-036 cannot receive Cycle 2 acceptance credit until the repaired default-branch judge is independently reviewed and proven in hosted execution against the exact candidate. | At the Cycle 2 planning snapshot `master` had advanced beyond `f5d6434d`; this document does not treat the unbound commit-distance count as current evidence. The prior Cycle 1 judge SHA is superseded for pinning; a repaired successor commit is pending, so this register does not invent one. | The working-tree repair and local diagnostics are authored evidence only. | **Pending.** Missing required candidate-repository context now refuses in the authored repair, but acceptance awaits independent review and hosted proof. | Cycle 2 prerequisite through P1 and P7; not closed by local diagnostics. | Owner + CI/evidence owner |
...
For Cycle 2, OC-035, OC-036, and OC-038 are checkpoint prerequisites until the
pending repaired default-branch judge is independently reviewed, pinned, and
proven by the exact candidate-bound and hosted P7 evidence. Their eventual
