...
| OC-020 | substrate | Candidate immutability during execution | **OPEN AGAIN. Two attempts have now failed, for opposite reasons.** (1) A read-only export was defeated by the candidate simply restoring the permissions it owns — mutate, execute the mutated bytes, restore bytes and modes, receipt PASS. (2) An authority boundary — low integrity on Windows, UID `nobody` on Linux — did defeat every attack, but **took all eight hosted CI jobs red**, including `core`, which had been green: the candidate suite exited 4 in 21 seconds on `ubuntu-latest` with no receipt produced at all. Both layers are withdrawn; the post-hoc check is what remains. *** THE WINDOWS CAUSE IS NOW IDENTIFIED, and it was the reason attempt 2 could not be repaired. *** A debugger-based loader trace on a hosted `windows-latest` runner (child launched with `DEBUG_PROCESS`, every `LOAD_DLL` recorded, unrestricted token as control) showed the restricted child loading `bcrypt.dll` and then dying on **`bcryptprimitives.dll`** — the CNG primitives provider, which CPython pulls in at interpreter startup for hash randomisation and `os.urandom` seeding, before any user code runs. That is why stdout and stderr were always empty. Six candidates were eliminated by measurement first: process creation (cmd.exe ran, exit 7), a System32-only effect (git.exe ran, its own exit 128), window station / desktop DACLs (S-1-5-12 already holds full access to both), privilege stripping (CPython runs on two privileges), the interpreter-directory ACL (granted, still `0xC0000142`), and finally the token variants themselves — `WRITE_RESTRICTED` alone kills CPython while privilege stripping alone does not. **`\Device\KsecDD`, the kernel conduit CNG seeds through, grants Everyone `0x1201bf` and S-1-5-12 only `0x1200a9` — an explicit ACE with exactly the four write bits stripped**, which is precisely the shape that passes a normal access check and fails `WRITE_RESTRICTED`'s restricting-SID second pass. *** A PROPOSAL TO GRANT THOSE FOUR BITS WAS REJECTED ON REVIEW and is recorded here as a rejected design alternative, not a pending option: device write rights determine which IOCTLs the I/O manager dispatches to a driver, no authoritative contract for the user-mode KsecDD interface proves every newly reachable operation is irrelevant, and matching the Everyone ACE is not exculpatory because the restricting-SID pass exists precisely to make Restricted Code narrower than Everyone. Classified as a real weakening of the wider sandbox, insufficiently bounded to accept, and within the spirit of relaxing the restriction merely to start Python. *** **A REPLACEMENT MECHANISM NOW MEASURES 10/10 ON A HOSTED WINDOWS RUNNER** (run `30425937950`): Mandatory Integrity Control alone — no `WRITE_RESTRICTED`, no restricting SID, no kernel-device change. CPython STARTS, and export write, ACL rewrite, rename, delete-recreate, hardlink and new-file are all refused while the state root stays writable and a nested subprocess works. The boundary comes from the integrity LABEL, so CNG never meets the second access-check pass. On Linux, Landlock plus a launcher-resolve ordering fix already satisfies constraints 1-3, evidenced by `alias_export_read_only`, `copy_writable` and **`strict_descriptor_read: "ok"`** — the exact check that failed in attempt 2. **WHERE THE APPARATUS LIVES, so every figure above can be re-derived rather than taken on trust:** the Windows measurements come from four diagnostic modules and one workflow carried on the remote branch `scratch/oc020-windiag` — a token-variant discriminator, a DEBUG_PROCESS-based loader tracer (which named the failing DLL), a Mandatory Integrity Control boundary probe, and the workflow that runs all three. The Linux measurements are on `scratch/oc020-landlock`. *** Deliberately NOT carried on this branch: they are diagnostic apparatus rather than deliverable, and landing them would enlarge the reviewed artifact for a row that is still open. The consequence must be stated rather than left implicit — if those branches are deleted, every figure above becomes unreproducible, so they must outlive this row. *** | A candidate can mutate a committed file, execute the mutated bytes, restore the original bytes, and receive a PASS receipt carrying the correct commit, tree and manifest digests. | Reproduction: chmod-restore yields `returncode=0`, `EXECUTED_MUTATED_BYTES` in stdout, receipt PASS. Withdrawal evidence: run `30405017655` at `c7e5969a`, 8/8 jobs failed, candidate suite exit 4 before any test ran. | **The constraint both attempts taught us:** the boundary must be one the candidate cannot revoke, AND it must not follow legitimate work out of the export. `shutil.copytree` uses `copy2` and preserves permission bits, so a read-only export propagates into the temp copies that guard tests legitimately mutate. Any next attempt must be proven on HOSTED `windows-latest` and `ubuntu-latest` before landing, not on a local fixture. | **No.** The mutate-execute-restore interval is reachable again. | An immutability boundary the candidate cannot revoke, demonstrated green on both hosted runners. *** THE MECHANISM NOW EXISTS AND IS MEASURED; WHAT REMAINS IS INTEGRATION AND PROOF, NOT DISCOVERY: move MIC from the standalone probe into the sandbox path, run the full G1 attack battery on BOTH hosted runners, and obtain independent review. Not closed by waiver, by the Linux result alone, or by treating unavailable Windows execution as a pass. *** **BLOCKS-CHECKPOINT** — this row is listed as a clause-4 merge blocker in the classification section below, and BLOCKS-DEPLOYMENT is defined here as *may remain open*. Carrying the softer tag made the register contradict itself in the permissive direction. | CI/evidence owner |
