...
`master` is **four commits beyond `f5d6434d`**. The Cycle 1 judge SHA is no
longer the pin for Cycle 2; its repaired successor is pending and no SHA is
claimed here. The protected CI lock is version-pinned without artifact hashes.
...
disclosure row under its recorded workflow-layer narrowing. Closed OC-021,
OC-029, OC-032, and OC-033 retain their existing dispositions.
