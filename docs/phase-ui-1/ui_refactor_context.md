...
| K4 | **Direct Keithley control + custom-command popup (NEW)** | Источник мощности tab + new popup surface | Direct control (P/V/I, Start/Stop/Emergency, A+B) was never accompanied by a custom-command surface in legacy GUI. Vladimir decided to add as new feature in Phase II.6: popup dialog with command catalog + free-text input + send + response display. Not strict preserve; tracked here as scope reminder. |
...
| K7 | **Phase Detector hints in GUI (engine plugin exists, GUI surface to be added)** | Background analytics → Phase II.1 Analytics overlay tile + Phase II.9 phase pill highlight | Engine-side plugin at plugins/phase_detector.py publishes detected_phase + phase_confidence as analytics/phase_detector/* readings. No GUI consumer currently. Vladimir decided to wire in Phase II as: (1) Analytics overlay tile showing detected phase + confidence, (2) Subtle highlight on suggested-next-phase pill in ExperimentOverlay phase stepper when confidence > threshold (threshold TBD in Phase II spec). |
...
### Note on K4/K7 evolution

K4 and K7 are listed in this preserve table for traceability, but per Phase 0 verification
(`docs/phase-ui-1/preserve_features_verification.md` commit `1e5ceb9`):
- K4 is partially new feature (custom-command popup), not strict preserve
- K7 is currently engine-only; GUI integration is part of Phase II scope, not preserve

This is intentional. Refactoring revealed gaps that we choose to fill rather than skip.
