...
**Status:** Living roadmap. Reflects reality as of HEAD `e4a60f3` (2026-04-19). Phase II blocks shipped end-to-end: II.1 AnalyticsView (`860ecf3`), II.6 Keithley (`96adf5a`), II.3 OperatorLog (`9676acc`), II.2 ArchiveOverlay (`e4a60f3`). II.6 post-review established the Host Integration Contract pattern codified in `PROJECT_STATUS.md`. Next block: II.5 ConductivityOverlay.
...
**Status:** ✅ COMPLETE (`e4a60f3`, 2026-04-19). Full-featured experiment archive overlay at `shell/overlays/archive_panel.py` — filter bar (template / operator / sample / date range / report presence / sort), 9-column list table with FONT_MONO timestamps, details panel (summary + metadata + notes + runs + artifacts + results + open-folder/PDF/DOCX/regenerate actions), and K6 mandate «Экспорт данных» card with CSV / HDF5 / Excel bulk exporters running in QThread workers that wrap existing `cryodaq.storage.*_export` classes verbatim. Emoji pictograms replaced with ASCII tags `[ДАННЫЕ]` / `[ИЗМЕРЕНИЯ]` / `[УСТАВКИ]`. Host Integration Contract wired: `_tick_status` mirror + `_ensure_overlay("archive")` replay; `on_reading` is a documented contract no-op (engine has no `analytics/experiment_finalized` broker event). Legacy widget marked DEPRECATED. Three Codex amend cycles on the initial landing: (1) HIGH — removed `__init__`-time refresh to avoid cold-start ZMQ race, deferred to first `set_connected(True)` transition when `_entries` empty; (2) MEDIUM — rewrote export worker as `QThread` subclass with `parent=self` ownership after `QObject+moveToThread+deleteLater` chain segfaulted in `QThread::started` delivery, added plain-Python stub exporter tests (MagicMock across the PySide signal boundary also crashed); (3) MEDIUM — added `_refresh_in_flight` guard against reconnect-flap race spawning duplicate refresh workers before the first result returns.
...
- **2026-04-19** Block II.2 — ArchiveOverlay shipped (`8a7251b` → `b040631` → `e4a60f3` after three Codex amend cycles). Full-featured experiment archive surface with filter bar, details pane, action row, and K6 «Экспорт данных» card running bulk CSV / HDF5 / Excel through `QThread` workers that wrap existing `cryodaq.storage.*_export` classes verbatim. Emoji pictograms replaced with ASCII bracketed tags per RULE-COPY-005. Three post-review amends: (HIGH) cold-start refresh deferral — removed `__init__` refresh, first-connect guard; (MEDIUM) QThread lifecycle rewrite — replaced `QObject+moveToThread+deleteLater` chain (segfaulted in `QThread::started` after MagicMock+PySide signal interaction) with a `QThread` subclass + `parent=self` ownership and plain-Python stub exporter tests; (MEDIUM) `_refresh_in_flight` guard — prevents reconnect-flap race from spawning duplicate refresh workers. Final Codex verdict PASS at `e4a60f3`. Test baseline 1321 → 1368 (+47 net).
- **2026-04-19** CI budget rule codified in `CLAUDE.md` under «Ключевые правила». Initial block commits run the full `pytest -q` suite; amend-fix commits (post-Codex surgical patches, 1-3 files changed, <100 LOC delta) run only targeted tests + ruff. Rationale: amend diffs have small blast radius by definition; burning 10-15 min of pytest wall-clock on every amend is wasted tokens. Full-suite regression detection is not lost — it runs at the start of the next block's initial commit. Exception for amends that touch widely-imported modules (`main_window_v2.py`, `engine.py`, `safety_manager.py`) — judgment call.
- **2026-04-19** PROJECT_STATUS + roadmap synced to HEAD `e4a60f3` (II.2 closed; II.5 ConductivityOverlay next; test baseline 1368).
