...
| Candidate source-inventory text | 457,509 lines | Strict UTF-8/no-NUL text in the intended candidate inventory, excluding the one generated architecture SVG. |
| Delivered-tree text | 457,593 lines | Final source inventory plus the one surviving regenerated architecture SVG. |
...
| Candidate test Python | 220,407 lines | Candidate `tests` inventory. |
...
the requested comparison. The boundary also verifies that
`rev-parse --show-toplevel` resolves exactly to the requested repository and
fails closed when local configuration, repository, ancestry, or tree authority
is foreign, ambiguous, active, or unavailable.
