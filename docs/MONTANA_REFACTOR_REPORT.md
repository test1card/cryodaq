...
| Candidate source-inventory text | 423,650 lines | Strict UTF-8/no-NUL text in the intended candidate inventory, excluding the one generated architecture SVG. |
| Delivered-tree text | 423,734 lines | Final source inventory plus the one surviving regenerated architecture SVG. |
| Baseline test Python | 80,529 lines | Python under `tests`. |
| Candidate production Python | 150,021 lines | Candidate `src/cryodaq` inventory, not 134,090. |
| Candidate test Python | 200,613 lines | Candidate `tests` inventory. |
...
| Architecture source manifest | 1,139 | Every intended source file except the self-referential generated `docs/refactor/architecture-*.svg` and `docs/architecture-montana-important.svg` outputs. |
| Delivered-tree files | 1,140 | Source manifest plus the one surviving regenerated architecture SVG. |
