...
| Candidate source-inventory text | 437,832 lines | Strict UTF-8/no-NUL text in the intended candidate inventory, excluding the one generated architecture SVG. |
| Delivered-tree text | 437,916 lines | Final source inventory plus the one surviving regenerated architecture SVG. |
| Baseline test Python | 80,529 lines | Python under `tests`. |
| Candidate production Python | 151,855 lines | Candidate `src/cryodaq` inventory, not 134,090. |
| Candidate test Python | 210,191 lines | Candidate `tests` inventory. |
...
| Architecture source manifest | 1,167 | Every intended source file except the self-referential generated `docs/refactor/architecture-*.svg` and `docs/architecture-montana-important.svg` outputs. |
| Delivered-tree files | 1,168 | Source manifest plus the one surviving regenerated architecture SVG. |
