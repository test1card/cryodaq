...
| Candidate source-inventory text | 413,841 lines | Strict UTF-8/no-NUL text in the intended candidate inventory, excluding the one generated architecture SVG. |
| Delivered-tree text | 413,925 lines | Final source inventory plus the one surviving regenerated architecture SVG. |
| Baseline test Python | 80,529 lines | Python under `tests`. |
| Candidate production Python | 148,588 lines | Candidate `src/cryodaq` inventory, not 134,090. |
| Candidate test Python | 194,695 lines | Candidate `tests` inventory. |
...
| Architecture source manifest | 1,110 | Every intended source file except the self-referential generated `docs/refactor/architecture-*.svg` and `docs/architecture-montana-important.svg` outputs. |
| Delivered-tree files | 1,111 | Source manifest plus the one surviving regenerated architecture SVG. |
...
| `scripts/soak_mock_stack_runner.py` | No equivalent exact evidence owner | New ~3,824-line runner | Test/evidence tool, not production acquisition logic; intentionally explicit but should avoid becoming a second launcher. |
