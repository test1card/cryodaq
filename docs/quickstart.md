...
На свежей установке предиктор по умолчанию отключён
(`config/cooldown.yaml`: `cooldown.enabled: false`), поэтому отсутствующая
локальная модель не блокирует запуск CryoDAQ и не требует ручного шага при
первом старте. Интерфейс при этом явно показывает, что прогноз траектории
недоступен и основанная на нём траекторная тревога не сработает.

Включайте предиктор только после размещения проверенной модели этого стенда в
`data/cooldown_model/predictor_model.json`: затем установите
`cooldown.enabled: true`. Чужая или непроверенная модель не является
допустимым способом убрать предупреждение.
...
Для новой лаборатории это ожидаемое состояние: **предиктор охлаждения
недоступен, пока ваш стенд не обучит собственную модель.** Отдельного действия
на этом шаге не требуется — продукт честно показывает предиктор как
недоступный, а не подставляет чужую модель.
