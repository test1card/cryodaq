...
**Will not:**
- Rewrite archived, evidence, or verbatim-quoted text (repository note: in this repository, `docs/campaigns/` archives, quoted owner/reviewer words, and anything fenced `<!-- verbatim:begin -->`…`<!-- verbatim:end -->` are byte-faithful records — an STE pass over them destroys exactly what they exist to preserve; see OB-012 in `docs/OBLIGATIONS.md`).
