...
|---|---|---|---|
| 2026-08-08 | guard-exercises-helper-not-production | Guards for the OC-030 migration drove `ChannelManager` rather than the seven production widgets; reverting a widget's selector left every guard green while a declared pressure channel was drawn as a temperature. **Second instance** — see PR #18, where the guard measured the writer only. **Promotion landed in this slice, not merely claimed:** `tools/unguarded_production_files.py` reverts every production file a change touches, one at a time, and reports which leave the suite green. An earlier version of this row said the promotion was "taken" while the script existed only in a scratch directory — which is the false assurance this ledger is supposed to prevent, written into the ledger itself. Review caught it. | the production-site guards added in PR #19 (not yet on this branch, so the path is deliberately not cited: the dead-path guard is right to refuse it) |
