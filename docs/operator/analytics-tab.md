...
| Измерение | Прогноз стационарной температуры | R тепловое (прямой эфир) | Каналы температуры |
...
### Прогноз стационарной температуры (фаза измерения, основной слот)

Отображает cold-stage температуру и асимптотический прогноз стационарного
состояния. Виджет получает поток Т12 через `set_cold_temperature_reading`.

### R тепловое (фаза измерения, справа сверху)
