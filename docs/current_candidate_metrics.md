<!-- Generated by tools/generate_montana_architecture_svgs.py; do not edit. -->
# CryoDAQ current-candidate metrics

This is the sole numeric authority for current-candidate aggregate measurements cited by the Montana report. Both this block and the surviving architecture SVG were rendered from the same frozen Git-index source snapshot; generated outputs are excluded from that snapshot to prevent self-reference.

| Measure | Value |
|---|---:|
| Baseline commit | `f5d6434d20dffae62c9f03fbc12f68b03f48351b` |
| Source snapshot tree | `458deefc08ec5d1b2cccb1e2bcec34f35f37cc1b` |
| Source snapshot object manifest SHA-256 | `1f9d2e88079475b51e2b3a71f207e0bcc69a9b93e09967e3aa4bb001114d38ec` |
| Changed source-inventory paths | 865 |
| Source-inventory Git churn | 313,841 insertions / 21,151 deletions / 292,690 net |
| Delivered-tree changed paths | 867 |
| Delivered-tree Git churn | 313,962 insertions / 21,151 deletions / 292,811 net |
| Changed binary paths, source / delivered | 9 / 9 |
| Baseline repository text | 188,539 lines |
| Candidate source-inventory text | 481,229 lines |
| Delivered-tree text | 481,350 lines |
| Baseline production Python | 68,857 lines |
| Candidate production Python | 157,558 lines |
| Production-Python inventory growth | 88,701 lines |
| Baseline test Python | 80,529 lines |
| Candidate test Python | 236,505 lines |
| Test-Python inventory growth | 155,976 lines |
| Baseline files | 779 |
| Candidate source-manifest files | 1,220 |
| Delivered-tree files | 1,222 |
| Baseline text / binary files | 767 / 12 |
| Baseline binary bytes | 3,878,632 |
| Candidate source text / binary files | 1,199 / 21 |
| Candidate source binary bytes | 5,812,612 |
| `src/cryodaq/engine.py`, baseline / candidate | 4,103 / 7,977 lines |
| `_run_engine`, baseline / candidate | 1,945 / 1,327 lines |
| Engine local-module imports, baseline / candidate | 69 / 58 |
| `scripts/soak_mock_stack_runner.py` | 3,901 lines |
| `scripts/soak_mock_stack.py` | 2,689 lines |
| Generated metrics document / surviving SVG | 37 / 84 lines |
