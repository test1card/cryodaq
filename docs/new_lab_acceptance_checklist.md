...
This is an offline observation procedure; it does not use `soak_mock_stack.py`, inject faults, or issue instrument commands. Before the real run, identify the four process roles (`launcher`, `engine`, `bridge`, and `assistant`) with the lab-approved external process observer. Every 5 s from startup through the selected 12 h or 72 h duration, that observer must append one UTF-8 JSON object to `samples.jsonl`. Each object has exactly `elapsed_s`, `wall_time`, and `roles`; `wall_time` is an offset-aware ISO-8601 timestamp, and `roles` has exactly those four role keys. Each role value has exactly integer `pid`, `started_ns`, `epoch`, `rss_bytes`, `threads`, and `descriptors`. Use `epoch: 0` for this no-restart soak, preserve the observer's process start identity in `started_ns`, and record actual descriptor counts rather than a substitute or unavailable value. An absent role, an unavailable descriptor count, or a cadence gap above 7.5 s leaves G7.1 **NOT PASSED**.
...
For a 72 h record, replace `profile('12h')` with `profile('72h')`. Preserve the exact `samples.jsonl`, command output, source commit, selected profile, and observer identity with the lab record. A nonzero exit status or any printed error is **NOT PASSED**; the mock-stack evidence validator is not evidence for this real-instrument gate.
