...
### G7.1 real-stack resource capture and verdict

This is an offline observation procedure; it does not use `soak_mock_stack.py`, inject faults, or issue instrument commands. Before the real run, identify the process roles the accepted production configuration actually starts with the lab-approved external process observer. `launcher`, `engine`, and `bridge` are always present; `assistant` is present only when `launcher.py::_assistant_runtime_required()` is true (LLM, automatic reporting, or periodic PNG enabled), so the observed role set is the production topology's set, not the mock soak's fixed four. Every 5 s from startup through the selected 12 h or 72 h duration at minimum — and through the full intended run duration when it exceeds the selected profile, so a week-long run keeps sampling past 72 h — that observer must append one UTF-8 JSON object to `samples.jsonl`. Each object has exactly `elapsed_s`, `wall_time`, and `roles`; `wall_time` is an offset-aware ISO-8601 timestamp, and `roles` has exactly the keys of the observed role set. Each role value has exactly integer `pid`, `started_ns`, `epoch`, `rss_bytes`, `threads`, and `descriptors`. Use `epoch: 0` for this no-restart soak, preserve the observer's process start identity in `started_ns`, and record actual descriptor counts rather than a substitute or unavailable value. A role the topology starts but that is absent from a sample, a role the topology does not start but that appears, an unavailable descriptor count, or a cadence gap above 7.5 s leaves G7.1 **NOT PASSED**. No real-stack observer invocation is checked in: the only `samples.jsonl` producer is the locked mock-stack runner, so the schema and verdict command above describe a requirement rather than a closable procedure. Until a validated observer invocation is provided and reviewed, G7.1 stays explicitly open and **NOT PASSED**.

After stopping observation, evaluate the captured file on the same checked-out source tree. From the repository root, run:

```powershell
$env:PYTHONPATH = $PWD
python -c "import json, sys; from pathlib import Path; from scripts.soak_mock_stack import evaluate_resources, profile; samples = [json.loads(line) for line in Path('samples.jsonl').read_text(encoding='utf-8').splitlines()]; errors = evaluate_resources(samples, profile('12h')); print('PASS' if not errors else '\n'.join(errors)); sys.exit(bool(errors))"
```

For a 72 h record, replace `profile('12h')` with `profile('72h')`. Preserve the exact `samples.jsonl`, command output, source commit, selected profile, and observer identity with the lab record. A nonzero exit status or any printed error is **NOT PASSED**; the mock-stack evidence validator is not evidence for this real-instrument gate. The shipped `evaluate_resources()` validates the mock soak's fixed four-role schema, so a production topology whose started role set differs from those four roles cannot yet be scored by it; that is part of why G7.1 stays explicitly open until a topology-matching observer and evaluator are provided and reviewed.

The channel-gap clause is a separate verdict that `evaluate_resources()` cannot decide, because its input contains only process-resource samples and no channel data. A channel gap is an interval between consecutive persisted timestamps for one `channel_id` that exceeds that channel's configured `poll_interval_s` (G1.4's timing bound). Query the stored rows per `channel_id` over the observed interval, the same query shape as G2.1; the maximum inter-arrival gap per channel must be at or below its configured `poll_interval_s`, and any larger gap is unexplained unless the observer recorded a restart or a documented instrument pause. An unexplained channel gap leaves G7.1 **NOT PASSED** even when `evaluate_resources()` is clean.
...
G7.1:
  ...
  profile_selection: do not select the slope-free short profile for G7.1; use the smallest long profile whose duration covers the intended run, and observe for that selected profile's full duration so evaluate_resources can validate coverage: 12 h for an intended run from 15 min through 12 h, 72 h for an intended run longer than 12 h through 72 h; for an intended run longer than 72 h use the 72 h profile, observe through the full intended run duration (sampling continues past the profile's 72 h to the run's end), and record that the limits applied are tighter than the run demands; never interpolate a limit between profiles
