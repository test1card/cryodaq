...
EXCLUDE_DIRS = {
    ...
    "graphify-out",
    "graphify-out.stale-pre-merge",  # generated knowledge graph
...
for dirpath, dirnames, filenames in os.walk(ROOT):
    ...
    for fn in filenames:
        if fn in EXCLUDE_FILE_NAMES:
            continue
        if any(suf in fn for suf in EXCLUDE_SUFFIXES_IN_NAME):
            continue
        p = Path(dirpath) / fn
        cat = categorize(p)
        if cat is None:
...
print(json.dumps({key: dict(zip(["files", "code", "comment", "blank"], value)) for key, value in agg.items()}))
