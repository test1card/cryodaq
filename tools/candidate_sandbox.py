...
def _windows_grant_interpreter_read_execute() -> None:
    ...
    *** WHAT IS MEASURED VERSUS WHAT IS INFERRED, because these are not the same. ***
    ...
    interpreter_files = sorted(path for path in interpreter_root.iterdir() if path.is_file())
    # icacls.exe takes exactly one target name per invocation -- passing several
    # explicit paths on one command line is not "grant on each of these", it is a
    # malformed argument list, so every target gets its own icacls call.
    grants: list[tuple[Path, str, bool]] = [(interpreter_root, "(OI)(CI)RX", False)]
    grants.extend((path, "RX", False) for path in interpreter_files)
    dlls_root = interpreter_root / "DLLs"
    if dlls_root.is_dir():
        grants.append((dlls_root, "(OI)(CI)RX", True))
    for path, permission, recurse in grants:
        command_line = [
            "icacls.exe",
            str(path),
            "/grant",
            f"*S-1-5-12:{permission}",
            "/C",
        ]
        if recurse:
        ...
        if completed.returncode != 0:
            raise PermissionError(
                "failed to grant the write-restricted candidate token read/execute on the "
                f"CPython interpreter ({path}): {completed.stderr.strip()}"
            )
...
def _windows_run_restricted(cwd: Path, state_root: Path, command: list[str]) -> int:
    ...
    _windows_grant_interpreter_read_execute()
