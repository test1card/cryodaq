...
# The surviving architecture graph ships as durable product documentation
# beside docs/architecture.md, not in the campaign-named docs/refactor/
# directory (manifest SVG decision: the two before-* comparison maps and
...
def _is_generated_output(path: str) -> bool:
    if PurePosixPath(path).match("docs/refactor/architecture-*.svg"):
        return True
    # Relative-path literal, not derived from MONTANA_IMPORTANT_SVG/ROOT at
    # call time: ROOT is monkeypatched to a throwaway repo in
    # test_architecture_snapshot_is_bound_to_index_and_excludes_generated_outputs,
...
def generate() -> None:
    ...
    outputs = (
        ...
        ("montana", montana_snapshot, MONTANA_IMPORTANT_SVG, False),
    )
