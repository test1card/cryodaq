...
def _active_day_database(data_dir: Path) -> Path | None:
    """The day file the engine is writing, newest first.

    The day rolls at 03:00 rather than midnight, so today's date is not
    necessarily the open file. Pick by modification time and let the caller see
    which one was chosen rather than assuming.
    """

    candidates = sorted(data_dir.glob("data_????-??-??.db"), key=lambda p: p.stat().st_mtime)
...
def main() -> int:
    ...
    print(f"database:  {database}  (mode=ro)")
