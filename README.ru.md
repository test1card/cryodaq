...
**Сбор данных, управление и безопасность для криогенной лаборатории.**
...
Все снимки ниже — реальный интерфейс, отрисованный из кода этого репозитория. На всех показано
состояние **без подключённого движка** — и это намеренно: именно там видно главное проектное решение.

<img src="docs/assets/screenshot-operator-display.png" alt="Сводка смены" width="820">

Сводка смены отвечает на вопросы, которые оператор задаёт на самом деле — *можно ли продолжать?*,
*что требует внимания?* — а когда ответить не может, прямо об этом говорит. У каждой карточки есть
явный бейдж **НЕТ СВЯЗИ** и название следующего безопасного шага. Правдоподобный ноль не рисуется
никогда.

<img src="docs/assets/screenshot-main-window.png" alt="Главное окно" width="820">

Главное окно: шестнадцать каналов, история температуры и давления, журнал оператора. Недоступные
каналы показывают `—` / «Нет данных», а не `0.0`.

<img src="docs/assets/screenshot-keithley-panel.png" alt="Управление источниками Keithley" width="760">

Управление опасными источниками. При отсутствии связи канал показывает **НЕИЗВЕСТНО**, а не `OFF`, и
кнопки заблокированы, потому что команду отправлять некуда. Оператор, прочитавший «OFF», подойдёт к
железу, которое может быть под напряжением; эта панель отказывается так лгать.

<img src="docs/assets/screenshot-calibration-panel.png" alt="Калибровка датчиков" width="760">

Калибровка датчиков по трём приборам LakeShore, сгруппированная по железу, которому они принадлежат.
...
- **Релизная основа:** последний локально доступный релизный тег — `v0.64.1`
  (2026-07-08).
- **Кампания качества:** крупный рефакторинг программной готовности к
  лаборатории влит в `master` 2026-07-31 после независимого ревью. Этот README
  не утверждает текущую branch или candidate SHA; live boundary задаёт
  `PROJECT_STATUS.md`. Merge — это только программный checkpoint: зелёный
  ordinary CI или mock-тесты сами по себе не означают лабораторную приёмку.
...
## Рефакторинг программной готовности

Рефакторинг (влит 2026-07-31; архив кампании — `docs/campaigns/`) сохраняет
панорамный, насыщенный информацией операторский workflow и движет CryoDAQ к
более узким владельцам, явным доказательствам и видимым границам отказа. Это
направление с открытыми гейтами приёмки, а не уже принятое системное свойство. В production acquisition выбранный для архива
...
влитым checkpoint-поведением, ещё ожидающим физической приёмки. Checkpoint
defaults и hardening не являются утверждением о релизе или физической приёмке;
см. раздел «Статус».
...
«текущему» эксперименту. Author/source назначает сервер. Клиент передаёт
`Idempotency-Key` из ровно 32 строчных шестнадцатеричных символов и повторно
использует тот же ключ для согласования и повторных попыток; сервер копирует
его в `request_id`. Клиенты не передают `request_id` в JSON. Публичные
...
Для `POST /api/v1/log` HTTP 409 означает доказанное отсутствие commit (следуйте
`retry_safe`), 502 означает неизвестный исход (не повторяйте вслепую и не
создавайте новый ключ), а 503 означает commit с ожидающей публикацией; действует
нормативный [контракт settlement/retry](docs/protocol.md#post-apiv1log-settlement-and-retry).
...
Активные конфигурационные файлы текущего checkpoint:
...
по умолчанию модель gemma4:12b через Ollama; в описанном
лабораторном развёртывании другие модели не установлены). Никаких внешних API.
...
Ограничения ниже относятся к текущей границе v0.64.1/checkpoint.
