...
class AnalyticsPlugin(ABC):
    ...
    def teardown(self) -> None:
        """Освободить ресурсы плагина перед выгрузкой / перезагрузкой.

        Вызывается :class:`PluginPipeline` перед удалением или заменой
        плагина (горячая перезагрузка).  Переопределите, чтобы закрыть
        файловые дескрипторы, остановить потоки или освободить иные
        ресурсы уровня модуля.  Реализация по умолчанию — no-op.
        """
