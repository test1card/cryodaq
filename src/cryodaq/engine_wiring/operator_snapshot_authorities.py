...
class AlarmEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if type(self.evaluator_error) is not bool:
            raise TypeError("evaluator_error must be exact bool")
        object.__setattr__(
            self,
            "experiment_id",
            _bounded_text(self.experiment_id, field="experiment_id", max_bytes=MAX_ID_UTF8_BYTES),
        )
