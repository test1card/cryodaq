...
class _SmuChannelBlock(QFrame):
    ...
    def _update_control_enablement(self) -> None:
        ...
        # able to see why Start/Stop are blocked before attempting a command,
        # not only after a rejected attempt. authorization_reason() already
        # computes this text; just surface it here and clear it once the
        # latch is reconciled.
        if self._unknown_outcome_requires is not None:
            unknown_outcome_reason = self.authorization_reason("keithley_start") or ""
            self._start_btn.setToolTip(unknown_outcome_reason)
            self._start_btn.setAccessibleDescription(unknown_outcome_reason)
            self._stop_btn.setToolTip(unknown_outcome_reason)
            self._stop_btn.setAccessibleDescription(unknown_outcome_reason)
        else:
