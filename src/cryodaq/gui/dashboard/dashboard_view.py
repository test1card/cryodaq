...
class DashboardView(QScrollArea):
    ...
    def __init__(
        ...
        # Connection/data cadence is observation only.  Mutations require a
        # separate, exact receipt from the live Engine/Safety owner.
        self._authority_source: str | None = None
    ...
    def set_authority_receipt(
        ...
        *,
        ...
        """Accept only a current live Safety-owner authority receipt.

        Readings and connection cadence never create this receipt.  Invalid,
        replay, stale, or disconnected projections revoke mutation authority.
        """
        valid = (
            type(producer_id) is str
            and bool(producer_id)
            and (experiment_id is None or (type(experiment_id) is str and bool(experiment_id)))
            and type(lifecycle) is SafetyLifecycle
            and type(readiness) is ReadinessTruth
        ...
        if not valid:
            ...
            self._update_mutation_authority()
            return
        if self._authority_revision is not None and revision <= self._authority_revision:
            return
        self._authority_source = producer_id
    ...
    def set_operator_snapshot(self, snapshot: object) -> None:
        """Project one ingress-qualified snapshot into mutation authority."""
        if type(snapshot) is not OperatorSnapshot:
            self._clear_authority()
        elif snapshot.cut.mode is not SnapshotMode.LIVE:
            self._clear_authority()
        else:
            self.set_authority_receipt(
                experiment_id=snapshot.experiment.experiment_id,
                producer_id=snapshot.cut.source,
                lifecycle=snapshot.readiness.lifecycle,
                readiness=snapshot.readiness.readiness,
    ...
    def _update_mutation_authority(self) -> None:
        ...
        if self._quick_log is not None:
            reason = (
                "Только чтение"
                if self._read_only
                else ("Нет подтверждённого Safety-owner" if not self._authority_valid else "Нет связи с Engine")
            )
    ...
    def _on_phase_transition_requested(self, phase: str) -> None:
        ...
        if type(experiment_id) is not str or not experiment_id:
            ...
            return
        if self._authority_experiment_id != experiment_id:
            self._phase_widget.set_operation_state(
                "error",
                "Текущий Safety-owner не подтвердил этот experiment_id; команда не отправлена",
            )
            return
    ...
    def _on_log_entry_submitted(self, message: str) -> None:
        ...
        if self._read_only or not self._connected or not self._authority_valid or not message:
            return
