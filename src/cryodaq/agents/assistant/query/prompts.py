...
— Прямая заметно лучше остальных (в разы) — поток постоянен: он не спадает по
  мере расходования источника. Это подпись натекания через щель: перепад
  снаружи почти не меняется, пока в камере намного меньше атмосферы.
— √t или ln t лучше прямой — источник спадает по мере истощения. Это подпись
  газоотделения и десорбции.
