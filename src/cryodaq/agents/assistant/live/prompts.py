...
- Конкретные значения, не размытые описания.
- Тон разговорный, не формальный (это сводка для оператора, не отчёт).
- Если событий мало — короткая сводка (5-10 слов про то что всё стабильно).
- Если событий много — структурируй по категориям (тревоги / фазы / операторский журнал /
  калибровка).
- 100-200 слов максимум. Telegram-friendly Markdown (жирный, курсив — допустимо).
- Не используй LaTeX, формулы ($...$) и подобное. Для стрелок — →, ↑, ↓.
