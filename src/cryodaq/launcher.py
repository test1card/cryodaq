...
_LAUNCHER_CONSTRUCTION_EXCEPTION_MAX_CHARS = 2048
_LAUNCHER_CONSTRUCTION_STDERR_TAIL_MAX_CHARS = 4096
...
def _bounded_diagnostic_text(text: str, max_chars: int, *, keep_tail: bool) -> str:
    """Return diagnostic text within one explicit character bound."""

    if len(text) <= max_chars:
        return text
    retained_chars = max_chars - len(_DIAGNOSTIC_TRUNCATION_MARKER)
    if keep_tail:
        return _DIAGNOSTIC_TRUNCATION_MARKER + text[-retained_chars:]
    return text[:retained_chars] + _DIAGNOSTIC_TRUNCATION_MARKER


def _bounded_exception_message(exc: BaseException) -> str:
    """Keep a printable exception reason without allowing an unbounded record."""

    try:
        message = str(exc)
    except BaseException as formatting_failure:
        message = f"<unprintable {type(exc).__name__} message; formatter={type(formatting_failure).__name__}>"
    return _bounded_diagnostic_text(
        message,
        _LAUNCHER_CONSTRUCTION_EXCEPTION_MAX_CHARS,
        keep_tail=False,
    )


@dataclass(slots=True)
class _BoundedEngineStderrTail:
    """Retain only the latest bounded engine stderr text for startup diagnosis."""

    max_chars: int = _LAUNCHER_CONSTRUCTION_STDERR_TAIL_MAX_CHARS
    ...
    def append(self, text: str) -> None:
        with self._lock:
            ...
            self._text = _bounded_diagnostic_text(joined, self.max_chars, keep_tail=True)
...
def _pump_engine_stderr(
    ...
    tail: _BoundedEngineStderrTail | None = None,
) -> None:
        ...
        while True:
            ...
            if len(raw_line) > _MAX_ENGINE_STDERR_LINE_BYTES:
                while raw_line and not raw_line.endswith(b"\n"):
                    raw_line = owner.readline(_MAX_ENGINE_STDERR_LINE_BYTES + 1)
                bounded_failure = "engine stderr line exceeded the forwarding bound"
                if tail is not None:
                    tail.append(bounded_failure)
                stderr_logger.error(bounded_failure)
                continue
...
class LauncherWindow(QMainWindow):
    ...
    def __init__(
        ...
        *,
        ...
        self._engine_stderr_tail = _BoundedEngineStderrTail()
    ...
    def _run_construction_step(
        ...
        try:
            ...
            stderr_tail_owner = getattr(self, "_engine_stderr_tail", None)
            stderr_tail = (
                stderr_tail_owner.snapshot() if isinstance(stderr_tail_owner, _BoundedEngineStderrTail) else ""
            )
            logger.critical(
                "Launcher construction failed; phase=%s exception=%s message=%s engine_stderr_tail=%s",
                phase,
                type(exc).__name__,
                exception_message or "<empty>",
                stderr_tail or "<none captured>",
            )
            ...
            raise _LauncherConstructionHold(self, phase) from exc
    ...
    def _start_engine(self) -> None:
        ...
        stderr_tail = _BoundedEngineStderrTail()
        self._engine_stderr_tail = stderr_tail
        if self._replay_source is None:
        ...
        try:
            stderr_thread = threading.Thread(
                ...
                args=(stderr_stream_owner, stderr_logger, stderr_tail),
