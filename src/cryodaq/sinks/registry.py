...
class SinkRegistry:
    ...
    def load_config(self, config_path: Path) -> None:
        ...
        with config_path.open(encoding="utf-8") as fh:
            raw = yaml.load(fh, Loader=OwnedSafeLoader) or {}
        ...
        # Live finalization is observational and must never build or mutate
        # the RAG index. Index construction is an explicit offline CLI
        # operation; an old enabled configuration is rejected visibly.
        rag_cfg = sinks_cfg.get("rag_index") or {}
        if rag_cfg.get("enabled", False):
            logger.warning("live RAG index rebuild is disabled; run cryodaq-rag-index offline")
