...
def _configured_serial(resource_str: str) -> str | None:
    """Extract the configured VISA serial when the resource names one."""

    parts = resource_str.split("::")
    if len(parts) >= 4 and parts[-1].upper() == "INSTR":
...
class Keithley2604B(InstrumentDriver):
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            _, (_manufacturer, _model, idn_serial, _firmware) = _parse_keithley_idn_family(idn_raw, mock=self.mock)
            expected_serial = _configured_serial(self._resource_str)
            if not self.mock and expected_serial is not None and idn_serial != expected_serial:
                raise ValueError("*IDN? serial does not match the configured VISA resource")
