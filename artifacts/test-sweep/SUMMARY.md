...
| 31 | 2 | 92 | 0 | 22 | 9 | 1 | analytics widgets (summary/vacuum/R-thermal/trajectory). NUMERIC-DISPLAY-WEAK (value forwarded/stored vs rendered label/curve getData()) + MOCK-BYPASS (vacuum spies set_history/set_prediction → real plot never renders) + 1 tautology (R_thermal band re-derives formula); 0/5 clean |
| 32 | 2 | 99 | 0 | 7 | 8 | 8 | insight/steady-state/time-window/palette/theme/fonts. HIGH=test_fonts 2x source-grep, launcher_theme_switch 4x MOCK-BYPASS (mocked os.execv returns → "before execv" ordering unproven), steady-state predictor double-call. design_system_rules = legitimate governance scan (clean); 6/14 clean |
| 33 | 2 | 98 | 0 | 3 | 11 | 4 | overview/preflight/shift/theme. HIGH=preflight safety-gate not proven (inject _checks + _rebuild_checks_ui vs real async safety/disk responses; does a FAILED precondition block start?); WIDGET-CONTRACT private-handler calls; theme_tokens/loader/plot_style legitimate (clean); 3/8 clean |
| 34 | 2 | 91 | 0 | 2 | 11 | 2 | tray/vacuum-panel/cleanup/watchdog/plural/prediction/pressure. HIGH=zmq watchdog poll-reason source-grep + pressure-plot guarded-pass; value-blind curve getData() tests; tray/cleanup-sentinel/plural clean; 3/7 clean |
| **T2** | **2** | **1175** | **1** | **153** | **132** | **60** | **GUI FIND complete (batches 22-34, 13 batches): 346 findings. 1 CRIT (alarm-panel NaN, ledger item 11). Themes: WIDGET-CONTRACT-WEAK (private attrs/handlers vs rendered output), ACTION/SAFETY-CONTROL-WEAK (keithley/preflight call private vs click → wiring unproven), MOCK-BYPASS, value-blind getData/labels. These FEED A FUTURE FIX PASS — never fixed.** |
| **ALL** | **0-2** | **2206** | **2** | **173** | **182** | **94** | **ENTIRE SWEEP COMPLETE: 35 batches (0-34). tier-0+1 FIX+VERIFY done (committed); tier-2 GUI FIND done (findings only). 2 CRIT prod-gaps held for architect (ZMQ timeout item 1, alarm-panel NaN item 11).** |
