...
| -- | -- | -- | -- | -- | **COMMIT c805dcf: batches 11-15 (full suite 3250 pass), local, push held** |
| 16 | 12 | 12 | 0 | 61 pass | rag-audit/calibration/cooldown-predictor; atomic-write behavioral, downsample synthetic V-kink, numeric tests use literal minima not formula re-derivation, matplotlib subprocess check |
| 17 | 16 | 16 | 0 | 47 pass | analytics vacuum-trend/cooldown-service/phase/steady; fit tests assert model_type+params±10%, eta closed-form, phase exact sequence; config tests were already clean |
| 18 | 17 | 16 | 1 | 85 pass | integration/launcher/notifications. 🔐 SECURITY VERIFIED: secret_str now runtime __dict__ inspection — NO token leak (all 3 Telegram classes SecretStr-only). DEFERRED: diagnostic_alarm_pipeline needs src/ to expose _sensor_diag_tick closure |
| 19 | 6 | 6 | 0 | 38 pass | replay-predictor/telegram/replay-engine; predictor tests now drive real _predictor_fires (not mocked), escalation-cancel proves task cancelled, curve-pub asserts both channels+values |
| 20 | 10 | 7 | 3 | 55+2 pass | sinks/replay/reporting/root; report_generator asserts archived values, frozen-entry _dispatch, replay-phases safety-block. DEFERRED (need src/ extraction to test real path): shutdown-drain, summary-metadata-export, replay-predictor PUB handshake |
| 21 | 13 | 13 | 0 | 72 pass | END fix-pass. launcher/web/logging/zmq-bind/tools; web_dashboard patches real `_async_engine_command` (was stale gui.zmq_client.send_command) + exact payload/entries; launcher_signals spies QTimer.singleShot/_start_engine (prod never calls _restart_engine); engine_stderr proves close() not just remove; logging_setup blocks PySide6 via sys.modules for real import/except; diag_zmq calls main() not re-derived addr; zmq_bind drives _bind_with_retry through EADDRINUSE retry+backoff + runtime LINGER-before-bind; launcher_backoff calls _handle_engine_exit behaviorally; theme_menu asserts os.execv args. Independently re-verified 72 pass + ruff-clean. ZERO prod bugs |
| -- | -- | -- | -- | -- | **FIX PASS COMPLETE. Clean CI-mirror gate (no global sqlite var, ollama-report file deselected): 3248 pass / 0 fail / 3 skip. COMMIT: batches 16-20 = ee93591; batch 21 = this commit. local, push held.** |
