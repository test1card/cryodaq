# F28 Cycle 2 code review — GemmaAgent production code only

CryoDAQ safety-critical asyncio cryogenic DAQ. Commit 535cc95.

GemmaAgent (Гемма): local LLM agent via Ollama, subscribes to EventBus,
handles alarm_fired, generates Russian summaries for Telegram + operator log.
NEVER executes engine commands. Fails gracefully if Ollama unavailable.

Review: gemma.py (GemmaAgent), output_router.py, prompts.py, engine.py wiring.

PRODUCTION CODE DIFF (tests excluded):
```
...
Focus:
1. GemmaAgent start/stop/task lifecycle — correct? Memory leaks?
2. Rate limiting (Semaphore + hourly deque) — edge cases?
3. Error isolation — can alarm handler crash engine?
4. Engine wiring — init/start/stop placement correct?
5. Safety: any path that modifies engine state?

Verdict: PASS / CONDITIONAL / FAIL
Findings: Severity, file:line, description, fix. Max 1500 words.
