...
| **OC-040** | **BLOCKS-DEPLOYMENT; engine YAML loaders inherit PyYAML's mutable parsing tables. Unlike OC-037 and OC-039 this residual fails in the UNSAFE direction.** | State that four loaders now share `src/cryodaq/_owned_yaml.py` and that **50 bare call sites across 30 modules remain**, including `core/safety_manager.py`, `core/interlock.py`, `core/channel_manager.py` and `engine.py`. State the measured consequence rather than the mechanism: against the defect as it shipped, a documented public `add_path_resolver` call substituted the `cold_channel`, `warm_channel` and `reference_temp_channel` identities in the shipped `config/physical_alarms.yaml` **silently**, leaving the rest of the document intact and emitting no diagnostic. Do not describe the class as closed on the strength of the fixed loaders: fixing instances is not closing a class, and an earlier version of the register made exactly that error. |
...
<!-- phase-1-status=IN_PROGRESS : when every Phase 1 item below is merged, change
     ...
     THIS COMMENT MUST NEVER SPELL THE COMPLETED VALUE IN EXPLANATORY PROSE; the
     marker assignment above is where the value belongs once Phase 1 closes. A
     prose sentence used to spell it, and a marker trigger reading the file for
     it fired on that sentence while the spine was not merged. A guard in
     tests/docs/test_docs_freshness.py now pins the token to exactly one
     occurrence. -->
