...
| **OC-040** | **BLOCKS-DEPLOYMENT; engine YAML loaders inherit PyYAML's mutable parsing tables. Unlike OC-037 and OC-039 this residual fails in the UNSAFE direction.** | State that all current PyYAML load-family calls now use `src/cryodaq/_owned_yaml.py` and that the owned-loader AST inventory reports **zero bypasses in `src/cryodaq`**; the load-family migration is complete and the open part of the row is the pre-import ordering. State the measured consequence rather than the mechanism: against the defect as it shipped, a documented public `add_path_resolver` call substituted the `cold_channel`, `warm_channel` and `reference_temp_channel` identities in the shipped `config/physical_alarms.yaml` **silently**, leaving the rest of the document intact and emitting no diagnostic. Do not describe the class as closed on the strength of the completed migration: fixing instances is not closing the class — the pre-import ordering remains open for every migrated loader — and an earlier version of the register made exactly that error. |
