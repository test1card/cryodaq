...
`ccf15129` repairs it with two authorities that must not be conflated: `root`, the
materialized tree receipts are read from, and `git_repository`, the candidate's real
checkout used only to resolve objects. `require_git_resolution` is fail-closed and
keyed on the protected path alone, so a missing repository refuses rather than
silently degrading. It also pins `requirements-protected-ci-lock.txt` in
`_PROTECTED_PRODUCER_FILES`, and makes a failing protected run report *why* — it
previously exited 1 in eight seconds printing nothing, and diagnosing this defect
required downloading an artifact.

**P1 must qualify all of it.** The repair is authored and not independently
reviewed; local measurement does not close the review or hosted gates. Note also
that the reviewer who ruled on the repair's design is not thereby a reviewer of its
implementation — P1's two reviewers must be independent of the author of
`ccf15129`.
...
* **P3** reconciles the governance test, stages all final P2/P3 content, updates the frozen inventory
  rows in `docs/MONTANA_REFACTOR_REPORT.md`, stages those, then regenerates **only**
  `docs/architecture-montana-important.svg` from that final staged index. `docs/refactor/` outputs are
