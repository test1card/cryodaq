...
12. **Design system v5.0.0 canonical** — `docs/design-system/**` — единственный источник правды по UI в текущей ветке. v5.0.0 — breaking-релиз: `default_cool` становится темой GUI по умолчанию для любой установки без явного выбора темы, а её `ON_PRIMARY` изменён с `#e8eaf0` на `#141210` (измеренные пары — в `accessibility/contrast-matrix.md`); `warm_stone` остаётся доступен через `config/themes/` и site-local `config/settings.local.yaml`. Ось классификации канала (дескриптор найден / отсутствует / отклонён), добавленная в v4.1.0, уже отрисовывается в `SensorCell`, и записанные расхождения с целевым контрактом мигрируют в OC-008/OC-030. Релизный снимок v0.64.1 в таблице выше исторически фиксирует v1.0.1. Значения токенов берутся ТОЛЬКО из `theme.py`.
